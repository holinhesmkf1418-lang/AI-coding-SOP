...
## 当前任务：Task 1.2 初始化后端工程

**目标：** 搭建后端项目骨架，`uvicorn main:app` 能跑出空白API

**具体要求：**
1. FastAPI 项目骨架，Python 3.11+
2. 建立分层目录：routers/、services/、repositories/、models/、schemas/、core/
3. CORS 配置好（允许前端 localhost:5173 访问）
4. main.py 只挂一个健康检查端点 GET /api/health
5. 项目目录在 backend/ 子目录下

**验收标准：**
- uvicorn backend.main:app --reload 启动无报错
